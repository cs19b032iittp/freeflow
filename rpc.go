...
var protocolID = protocol.ID("/p2p/rpc/file")

func (f *FileService) GetChunk(ctx context.Context, args GetChunkArgs, reply *GetChunkReply) error {
	log.Println("Received a Get Chunk call")

	return nil
}

// func (f *FileService) SendChunk(ctx context.Context, args GetChunkArgs, reply *GetChunkReply) error {
// 	log.Println("Received a Get Chunk call")
...
func startServer(h *host.Host) {
	rpcHost := gorpc.NewServer(*h, protocolID)
